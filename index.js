...
function parseKQ(html) {
    ...
    //console.log(`/Department  /EID  /Name  /LogDay  /Status`);
    while (true) {
        let i=0;
        ...
        let m = rex.exec(html);
        if (m) {
            let ClockDate=date.format(new Date(`${m[4]}`),'YYYY-MM-DD');
            
            rowData.push({Department:`${m[1]}`,EID:`${m[2]}`,Name:`${m[3]}`,ClockTime:`${m[4]}`,ClockDate:ClockDate});
            duplicateData.push({Department:`${m[1]}`,EID:`${m[2]}`,Name:`${m[3]}`,ClockDate:ClockDate});
...
function ShowResult(){
    ...
    //     console.log(rowData[i].Department+' '+rowData[i].EID+' '+rowData[i].Name+' '+rowData[i].ClockDate+' '+rowData[i].ClockTime);
    // }    

    let removeduplicateData=lodash.uniqWith(duplicateData, lodash.isEqual);

    let cleanData = lodash.orderBy(removeduplicateData, ['Department', 'EID','Name','ClockDate'], ['asc', 'asc', 'asc','asc']);
    //for(let i=0;i<cleanData.length;i++){
    //    console.log(cleanData[i].Department+' '+cleanData[i].EID+' '+cleanData[i].Name+' '+cleanData[i].ClockDate);
    //}

    let firstlogtime=new Array();
    let lastlogtime=new Array();  

    for(let i=0;i<cleanData.length;i++){
        lastlogtime=lodash.find(rowData, cleanData[i]);
        firstlogtime=lodash.findLast(rowData, cleanData[i]);
        cleanData[i].firstlogtime=firstlogtime.ClockTime;
        cleanData[i].lastlogtime=lastlogtime.ClockTime;
        cleanData[i].Status='';
        if(cleanData[i].firstlogtime==cleanData[i].lastlogtime && cleanData[i].Status==''){
            cleanData[i].Status='刷卡一次 ';
        }
        if(new Date(cleanData[i].firstlogtime)>new Date(cleanData[i].ClockDate+' 08:50:59') && cleanData[i].Status==''){
            cleanData[i].Status=cleanData[i].Status+'迟到 ';
        }
        if(new Date(cleanData[i].lastlogtime)<new Date(cleanData[i].ClockDate+' 16:49:59') && cleanData[i].Status==''){
            cleanData[i].Status=cleanData[i].Status+'早退 ';
        }
        if(new Date(cleanData[i].lastlogtime)<date.format(new Date(cleanData[i].firstlogtime).getTime+9 * 60 * 60 * 1000) && cleanData[i].Status==''){
            cleanData[i].Status=cleanData[i].Status+'工时不足 ';
        }
        else if (cleanData[i].Status=='') {
            cleanData[i].Status='正常';
        }
        console.log(cleanData[i].Department+' '+cleanData[i].EID+' '+cleanData[i].Name+' '+date.format(cleanData[i].firstlogtime,'YYYY-MM-DD HH:mm:ss')+ ' '+date.format(cleanData[i].lastlogtime,'YYYY-MM-DD HH:mm:ss') +' '+cleanData[i].Status);
    }
}

function askAll() {
   inquire('2020-12-24', '2021-1-11', '李旭超', false,
    ...
    function() { 
        console.log(`/Department  /EID  /Name  /FirstLogTime  /LastLogTime  /Status`);        
        ShowResult();
